...
diff_genes <- read.table("output/stable4.txt", sep='\t', header=TRUE, stringsAsFactor=F)

# Read in Hallmarks gene sets
hallmarks <- gmtToG("input/h.all.v6.2.symbols.gmt.txt") 

# Read in GO BP gene groups to be used
load("output/GO_use.RData") # loads c5names_use and c5probes_use
...
stable4_for_heatmap <- diff_genes
stable4_for_paper <- diff_genes_for_paper

write.table(stable4_for_heatmap, "output/stable4_heatmap", sep="\t", row.names=F, quote=F)
write.table(stable4_for_paper, "output/stable4_paper.txt", sep="\t", row.names=F, quote=F)
